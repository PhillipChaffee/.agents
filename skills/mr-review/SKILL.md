...
- **Code review skill** (`skills/code-review/SKILL.md`) — provides the review methodology, severity labels, checklists, and feedback format. Used in **review-only mode** (see Step 6) so it produces the curated summary without prompting for walkthrough or fix application. Read it before reviewing.
...
Follow `skills/code-review/SKILL.md` in **review-only mode**: run the full reviewer + verifier + curated summary flow, then halt before any walkthrough or fix prompt. mr-review takes those findings and posts them as draft notes rather than applying them.
