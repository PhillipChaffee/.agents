...
Fable upgrade candidates (reviewer + reason) for the main chat to apply when launching
subagents.
...
Launch the planner-selected reviewers in one message using parallel Task calls. Pass each
reviewer the full plan text and its planner-authored focus brief.

The reviewers are native Cursor agents in `agents/`:
...
`agents/` inlined and the same model. Never skip a selected reviewer.

### When Task is unavailable

If the `Task` tool is missing (nested subagent, restricted toolset, some Cloud agents), **do not**
