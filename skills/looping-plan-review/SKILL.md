...
description: >-
  ...
  plan review, align architecture before implementing, or when /ship runs its
  plan_review phase.
enabled: true
id: looping-plan-review
...
- The user asks to "loop on plan review", "align the plan architecture", or "iterate until
  the plan is ready".
- `/ship` runs its `plan_review` phase.
...
Invoke `plan-review` (`skills/plan-review/SKILL.md`) in **review-only** and
...
Launch **Plan Review Implementer** (`pr-implementer.md`) with only the `fix-now` findings.
