...
2. An open `.agents/plans/*.plan.md` file → use that.
...
If a plan-steps rule is accessible from where the skill is running — already loaded as an always-applied rule, or on disk in the installed kit (e.g. `~/.agents/rules/plan-steps.md` or `~/.cursor/rules/plan-steps.mdc`) — verify the plan complies with it. If no plan-steps rule is accessible, skip this pass.
