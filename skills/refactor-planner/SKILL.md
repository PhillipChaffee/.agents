...
Lean SKILL.md; detail lives in on-demand reference files under `skills/refactor-planner/references/`:
...
- **Run the project's test command** on the relevant path to confirm green. If no tests exist or any fail, **Step 0 of the produced plan must be characterization tests for current behavior** — universal "test before refactor" principle (Fowler), codified locally in [`python-pytest`](../../rules/python-pytest.md).
...
- `subagent_type` for the first call: `"Refactor Code Scout"` (matches the `name` field in `agents/refactor-code-scout.md`)
- `subagent_type` for the second call: `"Refactor Placement Scout"` (matches the `name` field in `agents/refactor-placement-scout.md`)

Run both scouts on `cursor-grok-4.6-high-fast`. Pass the model explicitly. If a named scout type is
unavailable, use `generalPurpose` with its `agents/` definition inlined and the same model.
...
- **DB-column renames** (Rename Field on an ORM model) generate migrations and require the **expand-contract pattern** (Parallel Change): add new column → migrate code to use it → defer old → drop in follow-up release. Codified locally in [`engineering`](../../rules/engineering.md).
...
- **Type-annotate all functions** including tests (per [`python`](../../rules/python.md)).
- **Pydantic models for new DTOs** over `TypedDict` / `dataclass` / raw `dict`s (per [`python`](../../rules/python.md)).
- **Standalone functions over `@staticmethod`** (per [`engineering`](../../rules/engineering.md)).
- **Class section headers** `Properties` / `Public methods` / `Private methods` with the specific `===`/`---` comment style (per [`python-class-sections`](../../rules/python-class-sections.md)).
- **100-character line limit** (per [`python`](../../rules/python.md)).
- **No commit amends or force-pushes**, even on personal branches (per [`engineering`](../../rules/engineering.md)).
- **`models.TextChoices`** for Django string enum fields (per [`python`](../../rules/python.md)).

## Output format (executor-ready plan)

Produce the plan as markdown with the following sections, in order. If the chat is in plan mode, call `CreatePlan` with the plan body; otherwise return the markdown inline and offer to save to `.agents/plans/refactor_<target>_<hash>.plan.md`.
