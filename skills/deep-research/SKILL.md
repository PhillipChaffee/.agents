...
- If a named type is unavailable, use `generalPurpose` with its `agents/` definition
...
For Tier 3 (and Tier 2 when it fits), decide per your agent platform's canvas
skill, if one is available (Cursor: `/canvas`): build a canvas when the output is a
