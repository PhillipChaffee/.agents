...
description: >-
  Use as part of multi-agent plan review. Filters reviewer findings before any
  walkthrough or fix runs by re-examining each finding as a finding (confirmed /
  false_positive / needs_rephrase). Defaults to grok (may read the codebase to
  verify claims). Optional Fable upgrade when verifying a large, conflicting, or
  high-stakes finding set — Fable is thinking-only on the supplied plan and finding
