...
4. **Minimal changes** (per `rules/minimal-changes.md`): no refactors, no renames, no "while I'm here" cleanups. Every commit in the loop must be traceable to a specific confirmed finding.
...
Invoke `code-review` (`skills/code-review/SKILL.md`) in **review-only mode**. Its mandatory
...
Use only model slugs present in the current subagent tool. Never invent or silently substitute an
unavailable model. Reviewer dispatch and fallback follow the self-contained `code-review` skill.
...
2. **CI-style checks**: run the `ci-lint-test` skill (`skills/ci-lint-test/SKILL.md`) for each changed repo — lint, format check, type check, then the relevant test suite.
