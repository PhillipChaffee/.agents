...
- **Suppression citation** (required if and only if Verdict is `accepted`) — path to the project rule + a quoted clause that documents the deviation as deliberate. Example: `shared/.cursor/rules/package-layout/RULE.md` — "this package uses a flat layout… do not introduce a `src/` layout unless the user explicitly requests it."
